...
2. 環境変数の設定 (chezmoi apply 後は `~/.zshenv` で自動設定):
   ```bash
...
```
dot_config/       # XDG configs (~/.config/), includes zsh/ (modular zsh)
config/           # Non-XDG configs (~/config/), includes brew/Brewfile, claude/hooks
dot_claude/       # ~/.claude/ (Claude Code agents, hooks, settings)
dot_zshenv        # ~/.zshenv (sets ZDOTDIR=~/.config/zsh)
